...
class Agent2(BaseAgent):
    """
    对手Agent，从B池选择模型和Prompt
    """
    
    def __init__(self, game_type: str = "three_player_ipd", model_name: str = None, prompt_name: str = None, token_pool_type: str = "three_player_ipd"):
        super().__init__()
    ...
    def _initialize_model_and_prompt(self):
        ...
        if not self.model_name:
            self.model_name = self._random_select_model_from_pool("B")
        
        # 如果没有指定提示，从B池随机选择一个
        if not self.prompt_name:
            self.prompt_name = self._random_select_prompt_from_pool("B")
        
        # 加载模型配置
        self.model_config = self._load_model_config("B", self.model_name)
        
        # 加载提示
        self.prompt = self._load_prompt("B", self.prompt_name)
