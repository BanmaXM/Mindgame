...
from .renderer import create_game_str
...
class ColonelBlottoEnv(ta.Env):
    ...
    def _render_game_state(self) -> str:
        lines = []
        lines.append(f"[GAME] === COLONEL BLOTTO - Round {self.state.game_state['current_round']}/{self.num_rounds} ===")
    ...
    def _prompt(self, player_id: int, game_state: Dict[str, Any]) -> str:
        ...
        return (
            f"[GAME] You are {role} in a game of ColonelBlotto. Each round, you can allocate up to {self.num_total_units} units across fields: {', '.join(self.field_names)}\n"
            f"Format: '[A4 B2 C2]'\nWin the majority of fields to win the round!"
        )

    def step(self, action: str) -> Tuple[bool, ta.Info]:
        # 玩家行动段：身份前缀 + 分配指令
        role = "Commander Alpha" if self.state.current_player_id == 0 else "Commander Beta"
        player_action_text = f"[{role}]\n{action}"
        self.state.add_observation(
            from_id=self.state.current_player_id,
            to_id=self.state.current_player_id,
            message=player_action_text,
